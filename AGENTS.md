...
- **App tokens**: Server-side client credentials flow via `/api/token/app`. Supports both **client secrets** and **certificates from Azure Key Vault**. For certificates, signing happens via Key Vault's `CryptographyClient`—the private key never leaves Key Vault.
- **Credential management**: App tokens use credentials stored exclusively in Azure Key Vault. For secrets, the server fetches the value at runtime. For certificates, the server requests a signature from Key Vault (private key never leaves). No local `.env` credentials are used for app tokens.
...
- **Server-side Key Vault**: `src/lib/server/keyvault.ts` for secret fetching and certificate status, `src/lib/server/keyvault-signer.ts` for JWT signing via Key Vault's `CryptographyClient`, `src/lib/server/keyvault-validator.ts` for validating app configs, and `src/lib/server/msal.ts` for confidential client token acquisition using `@azure/msal-node`.
...
- **App Tokens**: Keep token exchange logic server-side (see `src/lib/server/keyvault.ts`, `src/lib/server/keyvault-signer.ts`, and `src/lib/server/msal.ts`). Certificate private keys never leave Key Vault.
