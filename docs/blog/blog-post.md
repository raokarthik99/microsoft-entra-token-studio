![Entra Token Studio Hero](assets/entra-token-studio-hero.png)
...
![Entra Token Studio Welcome](assets/welcome-page.png)
...
2. **Local SvelteKit server**: For secrets, fetches the value from Key Vault; for certificates, requests a signature from Key Vault (private key never leaves). Then exchanges it for a token via Entra ID.
...
![Connecting an app registration](assets/demo-app-setup.webp)
...
3. For secrets: server fetches the secret from Key Vault. For certificates: server requests a signature from Key Vault (private key never leaves)
4. Server exchanges the credential/signature for a token via Entra ID's **client credentials** flow
...
![App token issuance flow](assets/demo-app-token.webp)
...
![User token issuance flow](assets/demo-user-token.webp)
...
![Claims inspection and filtering](assets/demo-claims.webp)
...
![History with action menu](assets/demo-history.webp)
...
**App-Linked Entries:** History, favorites, and pinned tokens are all linked to their originating connected app. This means when you're looking at a history entry or favorite, you always know which app configuration was used—no ambiguity about tenant, client ID, or credentials.

From history, you can:

- **Load** a previous token to inspect it again
- **Reissue** a fresh token with identical parameters—the reissue flow automatically uses the correct connected app, even if you've switched contexts since then
...
![Pinned token in sidebar](assets/demo-pinned-token.webp)

The quick-pick input surfaces pinned favorites, recent tokens, and common presets.

![Quick-pick input showing favorites and presets](assets/demo-quick-pick.webp)
...
### Step 5: Settings & Data Management

The **Settings page** consolidates profile information, preferences, and data management in one place.

![Settings page walkthrough](assets/demo-settings.webp)

Key features:

- **Profile at a glance** — Your authenticated identity, tenant ID, object ID, and which app was used for sign-in. Useful when debugging multi-tenant scenarios.
- **Identity behavior** — Choose whether the app should reuse the last signed-in account or prompt each time. Useful for teams testing different user contexts.
- **Theme customization** — Light, dark, or follow system preference.
- **Data export/import** — Backup your history, favorites, and app configurations to transfer between devices or browsers. The export dialog includes explicit security acknowledgments (tokens are sensitive; delete after import; never share)—a deliberate speed bump to reinforce security habits.
- **Clear all data** — Factory reset with a detailed preview of what gets deleted.

This is the "housekeeping" layer—ensures your workflow state persists as needed while keeping data hygiene straightforward.
...
> 📖 **Prefer a detailed quickstart?** See the [README](https://github.com/raokarthik99/microsoft-entra-token-studio#readme) for step-by-step terminal and VS Code workflows, troubleshooting tips, and full prerequisites.
...
Your identity needs appropriate access to your Key Vault. Choose the role based on your credential type:

- **Key Vault Secrets User** — for client secrets
- **Key Vault Crypto User** + **Key Vault Certificates User** — for certificates (signing requires crypto access; the private key never leaves Key Vault)
- **Key Vault Administrator** — for both (broader access, useful for dev scenarios)
...
📚 [Microsoft Docs: Assign Key Vault access using RBAC](https://learn.microsoft.com/en-us/azure/key-vault/general/rbac-guide)

---

## Configuring Your Entra App Registration

Depending on which token flows you need, configure your app registration accordingly.

### For User Tokens (Delegated Permissions)

If you want to use the **user token** flow (Authorization Code + PKCE):
...
> ⚠️ Use the **SPA** platform type, not "Web". The Web platform has different CORS and implicit grant expectations that break MSAL.js browser flows.

> **Permissions Note:** To add a redirect URI, you must be an **owner** of the app registration or have admin privileges (e.g., Global Administrator, Cloud App Administrator). If you encounter permission issues, contact the app owner or your tenant administrator.

**2. Add Delegated Permissions**

In your app registration → **API permissions** → **+ Add a permission**, add the delegated scopes you want to test (e.g., `User.Read`, `Mail.Read`, `Files.ReadWrite`).

### For App Tokens (Client Credentials)

If you want to use the **app token** flow (service-to-service):

**1. Add Application Permissions**

In your app registration → **API permissions** → **+ Add a permission**, select **Application permissions** for the APIs you need. Grant **admin consent** after adding them.

**2. Set Up Credentials**

You can use either a **client secret** or a **certificate**:

_Using a certificate (recommended):_
...
_Using a client secret:_
1. In your app registration → **Certificates & secrets** → **+ New client secret**
2. Copy the secret value and store it in **Key Vault** as a secret
3. Reference the **Key Vault secret name** in Entra Token Studio

> **Security Note:** For certificates, the private key **never leaves Key Vault**. Signing happens via `CryptographyClient` — the server only retrieves the public certificate metadata and requests signatures.

> ⚠️ **Credential Expiry Reminder:** Both secrets and certificates expire. When rotating credentials:
> - **Secrets:** Update the Key Vault secret value to match the new secret from app registration
> - **Certificates:** Upload the renewed certificate's public key (.cer) to your app registration
> 
> Set calendar reminders before expiry—expired credentials cause authentication failures that can be tricky to diagnose. Consider [Key Vault auto-rotation](https://learn.microsoft.com/en-us/azure/key-vault/certificates/overview-renew-certificate) for certificates.
